...
void main()
{
    ...
    float dist1 = v_vLine1.z - dot(v_vLine1.xy, v_vPosition) + 30.0;
    float dist2 = v_vLine2.z - dot(v_vLine2.xy, v_vPosition) + 30.0;
    
    float dist = min(max(dist1, dist2), 0.0) + length(max(vec2(dist1, dist2), 0.0)) - 30.0;
